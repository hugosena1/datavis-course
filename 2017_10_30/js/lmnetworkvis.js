function Network() {
  ...
  function setupData(data) {
    ...
    data.links.forEach(function(l) {
      ...
      l.source.linkcount += 1;
      l.target.linkcount += 1;
    });
    ...
    circleRadius = d3.scale.linear().range([3, 21]).domain(countExtent);

    colorScale = d3.scale.linear().range(["#f5fffa","#00783c"]).domain([1,36]);

    data.nodes.forEach(function(n) {
      n.radius = circleRadius(n.linkcount);
      n.color = colorScale(n.linkcount);
    });
  ...
  function showDetails(d, i) {
    ...
    content += '<p class="main">' + "No. de links: " + d.linkcount + '</span></p>';
  ...
  network = function(selection, data) {
    ...
    updateLinks();
    // set the tick callback, charge and linkDistance
    force.on("tick", forceTick).charge(-300).linkDistance(75);
